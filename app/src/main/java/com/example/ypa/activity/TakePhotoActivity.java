...
import com.example.ypa.adapter.PhotoFiltersAdapter;
import com.example.ypa.view.RevealBackgroundView;
            ...
            if(grantResults.length > 0){
                for(int i= 0; i < grantResults.length; ++i){
                    if(grantResults[i] == PackageManager.PERMISSION_DENIED){
                        new AlertDialog.Builder(this).setTitle("알림").setMessage("권한을 허용해 주세요").setPositiveButton("종료", new DialogInterface.OnClickListener() {
                            @Override
